...
getMrpp <- function(countsWithProvince, presence,minCodes)
{
    ...
    sitesInProvince = count(sitesSample$type)
    provincesSingleSite = subset(sitesInProvince, freq<2)$x
    sitesSample <- subset(sitesSample, !type %in% provincesSingleSite)
    ...
    codeMrpp <- mrpp(presenceSample, sitesSample$type, distance="jaccard",weight.type=1)
#    print(codeMrpp)
    return (c(codeMrpp$delta, codeMrpp$A, codeMrpp$Pvalue, nrow(sitesSample)))
}

presence <- read.csv("../09_presence/presenceBritannia.csv", header=T, sep=';')
counts <- read.csv("../07_countSample/countsBritannia.csv", header=T, sep=";")
locations <- read.csv("../04c_euclidean/locationsBritannia.csv", header=T, sep=";")
counts$province <- locations$province
counts$type <- locations$type
countsWithProvince <-  subset(counts, type!= '')
